...
class USBFeedback(Layers):
    ...
    def __init__(self, pin, nLeds = 64, brightness=0.2):
        import random
    ...
    def updateLights(self):
        ...
        if self.currentLayer == 0:
            self.assignColorToLayerIndicator( pulsed(RED,PURPLE, self.pulseHighOn, self.wpmHigh))
            self.redLED.duty_cycle = dtcyc
            self.greenLED.duty_cycle = dtcycOff
            self.blueLED.duty_cycle = dtcycOff
                
        elif self.currentLayer == 1:
            self.assignColorToLayerIndicator( pulsed(GREEN,PURPLE, self.pulseHighOn, self.wpmHigh))
            self.redLED.duty_cycle = dtcycOff
            self.greenLED.duty_cycle = dtcyc
            self.blueLED.duty_cycle = dtcycOff
        elif self.currentLayer == 2:
            self.assignColorToLayerIndicator( pulsed(BLUE,PURPLE, self.pulseHighOn, self.wpmHigh)) 
            self.redLED.duty_cycle = dtcycOff
            self.greenLED.duty_cycle = dtcycOff
            self.blueLED.duty_cycle = dtcyc
        elif self.currentLayer == 3:
            self.assignColorToLayerIndicator( pulsed(YELLOW,PURPLE, self.pulseHighOn, self.wpmHigh))
            self.redLED.duty_cycle = dtcyc
            self.greenLED.duty_cycle = dtcyc
            self.blueLED.duty_cycle = dtcycOff
